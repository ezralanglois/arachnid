...
def reconstruct_nn4_3(gen1, gen2, align1=None, align2=None, npad=2, sym='c1', weighting=1, **extra):
    ...
    root = 1 if mpi_utility.size(**extra) > 2 else 0
    recon = eman2_utility.backproject_nn4(gen1, align1, npad=npad, sym=sym, weighting=weighting)
    mpi_utility.block_reduce(eman2_utility.em2numpy(recon[1][0]), root=root, **extra)
    mpi_utility.block_reduce(eman2_utility.em2numpy(recon[1][1]), root=root, **extra)
    if root != 0:
        mpi_utility.send_to_root(eman2_utility.em2numpy(recon[1][0]), root, **extra)
        mpi_utility.send_to_root(eman2_utility.em2numpy(recon[1][1]), root, **extra)
    recon1 = recon if mpi_utility.is_root(**extra) or mpi_utility.get_rank()==root else None
    
    root = 2 if mpi_utility.size(**extra) > 2 else 0
    ...
    if root != 0:
        ...
        elif mpi_utility.is_root(**extra) or mpi_utility.get_rank()==root:
            return eman2_utility.finalize_nn4(recon)
    else:
