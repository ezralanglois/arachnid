...
def configuration(parent_package='',top_path=None):  
    ...
    if 1 == 1:
        config.add_library('spider_utillib', sources=['spider_lib.f90'])#,  define_macros=[('SP_LIBFFTW3', 1)])
    else:
    ...
    config.add_extension('_spider_util', sources=['spider_util.f90'], libraries=['spider_utillib'])
    config.add_extension('_image_utility', sources=['image_utility.i', 'radon.c'], define_macros=[('__STDC_FORMAT_MACROS', 1)], depends=['image_utility.h'], swig_opts=['-c++'])
