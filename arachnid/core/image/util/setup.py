...
def configuration(parent_package='',top_path=None):  
    ...
    trans_src = 'transform_wrap.cpp' if os.path.exists(os.path.join(os.path.dirname(__file__), 'transform_wrap.cpp')) else 'transform.i'
    ...
    config.add_extension('_transform', sources=[trans_src], define_macros=[('__STDC_FORMAT_MACROS', 1)]+compiler_defs, depends=['transform.hpp'], swig_opts=['-c++'], extra_info = blas_opt, extra_compile_args=compiler_args, extra_link_args=compiler_args, libraries=compiler_libraries)
