...
template<class T>
T integrate(T* img_int, dsize_type r0, dsize_type c0, dsize_type r1, dsize_type c1, dsize_type icol)
{
    T sum=0.0;

    sum += img_int[r1*icol+c1];

    if ( (r0 - 1 >= 0) && (c0 - 1 >= 0) )
        sum += img_int[(r0 - 1)*icol + (c0 - 1) ];

    if ( (r0 - 1) >= 0)
        sum -= img_int[(r0 - 1)*icol + c1];

    if ( (c0 - 1) >= 0)
    	sum -= img_int[r1*icol + (c0 - 1)];
    return sum;
}
...
//#	endif
	for(dsize_type r=0;r<row;++r)
	{
		...
		for(dsize_type c=0;c<col;++c)
		{
			...
			T win_sum = integrate(img_int, r, c, r_end, c_end, icol);
			T win_sum_sq = integrate(img_int_sq, r, c, r_end, c_end, icol);
