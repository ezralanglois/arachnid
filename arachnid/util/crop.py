...
def setup_options(parser, pgroup=None, main_option=False):
    ...
    if main_option:
        ...
        parser.add_option("-o", output="",              help="Output filename for window stack with correct number of digits (e.g. sndc_0000.spi)", gui=dict(filetype="save"), required_file=True)
        parser.add_option("-s", coordinate_file="",     help="File containing coordinates of objects", gui=dict(filetype="open"), required_file=True)
        parser.add_option("-d", selection_doc="",       help="Selection file for a subset of good windows", gui=dict(filetype="open"), required_file=False)
