...
def reconstruct_SNI(spi, engine, input_stack, align, selection, curr_slice, vol_output, shared_scratch, local_scratch, **extra):
    ...
    if mpi_utility.is_root(**extra):
        dalamap = spider_utility.file_map(dala_stack, mpi_utility.size(**extra))
        dala_stack = spider.stack(spi, dalamap, mpi_utility.size(**extra), format_utility.add_prefix(local_scratch, "dala_full_"))
...
def setup_options(parser, pgroup=None, main_option=False):
    ...
    setup_options_from_doc(parser, 'bp_cg_3', classes=spider.Session)
    if main_option:
