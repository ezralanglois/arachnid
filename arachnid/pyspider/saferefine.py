...
def refine_volume(spi, alignvals, curr_slice, refine_index, output, resolution_start=30.0, num_iterations=0, aggressive=False, **extra):
    ...
    if mpi_utility.is_root(**extra):
        ...
        if os.path.exists(resolution_file):
            if not aggressive: min_resolution=min(25, resolution_start)
            res_iteration = numpy.zeros((num_iterations+1, 5))
            tmp = numpy.loadtxt(resolution_file, delimiter=",")
            res_iteration[:tmp.shape[0]]=tmp
            res_tmp, param['trans_range'], extra['angle_range'], extra['hp_radius'], resolution_next  = res_iteration[refine_index]
            param['trans_range']=int(param['trans_range'])
            if res_tmp > 0.0: resolution_start=res_tmp
            ...
            min_resolution=min(25, resolution_start)
            if aggressive:
                min_resolution = res_iteration[refine_index-1] if refine_index > 0 else 30
    ...
    min_resolution = mpi_utility.broadcast(min_resolution, **extra)
    ...
    for refine_index in xrange(refine_index, num_iterations):
        param['bin_factor']=extra['bin_factor'] = decimation_level(resolution_start, max_resolution, **param)
        ...
        #if param['bin_factor'] == 1.0: extra['interpolation'] = 'FS'
        extra['trans_range'] = max(extra['trans_range'], 2)
        extra['theta_delta'] = theta_delta_est(resolution_next, **extra)
        if not aggressive:
            extra['theta_delta'] = max(extra['theta_delta'], 0.5)
        if extra['theta_delta'] > 7.9: extra['angle_range'] = 0
        #extra['shuffle_angles'] = False #extra['theta_delta'] == theta_prev and refine_index > 0
        extra['min_resolution'] = min_resolution
        if mpi_utility.is_root(**extra):
            _logger.info("Refinement started: %d. %s"%(refine_index+1, ",".join(["%s=%s"%(name, str(extra[name])) for name in refine_name])))
            res_iteration[refine_index+1] = (0.0, extra['trans_range'], extra['angle_range'], extra['hp_radius'], resolution_next)
            numpy.savetxt(resolution_file, res_iteration, delimiter=",")
        resolution_last=resolution_start
        resolution_start = refine.refinement_step(spi, alignvals, curr_slice, output, output_volume, refine_index, target_bin=(param['bin_factor'], param), **extra)
        ...
        if mpi_utility.is_root(**extra): 
            if (resolution_last-resolution_start) < resolution_start/30:
                num_iter_unchanged +=1
            ...
            if refine_index > 0 and num_iter_unchanged > 1 and 1 == 0:
            ...
            if not aggressive:
                if min_resolution > 4 and extra['theta_delta'] < 1.01 and num_iter_unchanged>4:
                    num_iter_unchanged=0
                    if min_resolution > 10:
                        min_resolution -= 5
                    else:
                        min_resolution -= 2
            else:
                min_resolution = resolution_last
            
            b = decimation_level(resolution_start, max_resolution, **param)
            if not aggressive:
                if (trans_range/b) < 3:
                    resolution_next = resolution_next*0.75
                else:
                    resolution_next = resolution_start*0.8
            else:
                _logger.error("trans: %f -> %f "%(trans_range, trans_range/b))
                if refine_index > 0 and num_iter_unchanged > 3 and (trans_range/b) < 3:
                    resolution_next = resolution_next*0.9
                elif resolution_start < (resolution_last*0.7):
                    resolution_next=resolution_last*0.7
                else:
                    resolution_next = resolution_start*0.8
...
def decimation_level(resolution, max_resolution, apix, min_bin_factor, window, **extra):
    ...
    dec =  min(max(1, min(max_resolution/(apix*6), resolution / (apix*6))), min_bin_factor)#*0.75
