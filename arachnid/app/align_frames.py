...
def benchmark_in_memory(filename, gain_file="", bin_factor=1.0, invert=False, **extra):
    ...
    trans = align_triplet(fourier_frames, **extra)
    avg = average_fft(fourier_frames, trans)
    write_perdiogram(avg, 2, **extra)
    '''
...
def align_l1(fourier_frames, upsampling=2, search_radius=50, lowpass_sigma=None, **extra):
    '''
    '''
...
def _xcorr_dft_peak(f3, usfac, search_radius, y0=0, x0=0, tshape=None, template_ssd=None):
    ...
    '''
    if template_ssd is not None:
        image = tshape
        tshape=image.shape
        CC = scipy.fftpack.ifft2(f3).real.copy()
        CC = numpy.ascontiguousarray(_centered(CC, numpy.abs(numpy.array(f1.shape)-numpy.array(f2.shape))+1))
        similarity.normalize_correlation(CC, image, tshape, template_ssd)
        peak = CC.max()
    else: peak=0
    '''
